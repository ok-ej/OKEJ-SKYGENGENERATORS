...
public class SkygenCommands implements CommandExecutor, TabCompleter {
    ...
    private void showHelp(CommandSender sender) {
        ...
        if (sender.hasPermission("okejgenerators.admin")) {
            sender.sendMessage(plugin.getMessageUtils().formatMessage("&8→ &x&0&0&8&D&F/okgen reload &7- &fPrzeładuj konfigurację pluginu"));
            sender.sendMessage(plugin.getMessageUtils().formatMessage("&8→ &x&0&0&8&D&F/okgen genboost <ilość> <czas> &7- &fAktywuj boost generatorów"));
            sender.sendMessage(plugin.getMessageUtils().formatMessage("&8→ &x&0&0&8&D&F/okgen nadaj <nazwa> &7- &fNadaj genboosta"));
            sender.sendMessage(plugin.getMessageUtils().formatMessage("&8→ &x&0&0&8&D&F/okgen toggle <generator> &7- &fWłącz/wyłącz generator"));
            sender.sendMessage(plugin.getMessageUtils().formatMessage("&8→ &x&0&0&8&D&F/okgen list &7- &fLista generatorów"));
        }
    }
}